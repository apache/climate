...
def displayPrecip(finalMCCList): 
	...
	return 
#******************************************************************
def plotHistograms(finalMCCList):
	'''
...
def plotAccuInTimeRange(starttime, endtime):
	...
	return	
#******************************************************************
def createTextFile(finalMCCList):
	'''
	...
	for eachPath in finalMCCList:
		...
		print "duration is ", duration, type(duration), float(duration)
		...
		for eachNode in eachPath:
			...
			if thisNode['cloudElementArea'] >= OUTER_CLOUD_SHIELD_AREA:
				averagePropagationSpeed += findCESpeed(eachNode)
				speedCounter +=1
		...
		MCSUserFile.write("\nLife duration is %s " %(str(duration)))
		...
		MCSUserFile.write("\nMCS minimum preicip is %.4f " %(minCEprecipRate))
		MCSUserFile.write("\nMCS maximum preicip is %.4f " %(maxCEprecipRate))
		MCSUserFile.write("\nTotal precipitation during MCS is %.4f mm/lifetime" %(MCSPrecipTotal))
		MCSUserFile.write("\nAverage MCS precipitation is %.4f " %(avgMCSPrecipTotal))
		MCSUserFile.write("\nAverage MCS maximum precipitation is %.4f " %(avgMaxMCSPrecipRate))
		MCSUserFile.write("\nAverage MCS minimum precipitation is %.4f " %(avgMinMCSPrecipRate))
	...
	MCSSummaryFile.write("\nAverage startTime is %s " %(str(averageTime(startTimes))))
	MCSSummaryFile.write("\nAverage endTime is %s " %(str(averageTime(endTimes))))
	...
	MCSSummaryFile.write("\nMaximum propagation speed is %.4f ms^-1 " %(max(allPropagationSpeeds)))
	MCSSummaryFile.write("\nMinimum propagation speed is %.4f ms^-1 " %(min(allPropagationSpeeds)))
	...
	MCSSummaryFile.write("\nAverage precipitaion is %.4f mm h^-1 " %(avgPrecipTotal))
