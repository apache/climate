...
target_output_dir = '/nas/share4-cf/plean/CRU_TS_3.0/xml/'

# Create a list of files
myfile = datadir+'cru_ts_3_00.1901.2006.pre.nc'
...
for mytime in xrange(len(times)):
   ...
   n_days_in_month = calendar.monthrange(int(myyears[mytime]),int(mymonths[mytime]))[1]

   print data.data[mytime,:,:].min(), data[mytime,:,:].min(),data.data[mytime,:,:].max()
   # Unit conversion from mm/month to mm/day
   data[mytime,:,:] = data[mytime,:,:] / float(n_days_in_month)
   print data.data[mytime,:,:].min(), data[mytime,:,:].min(),data.data[mytime,:,:].max()

   iso_time = str(int(myyears[mytime]))+str("%02i" % mymonths[mytime])+'01T000000Z'
   print iso_time, n_days_in_month
